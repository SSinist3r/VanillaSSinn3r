...
class Process(AbstractProcess):
    ...
    def write_memory(self, base_address: int, write_buffer: ctypes_buffer_t):
        try:
            # print(hex(self.get_mem_info(base_address).Protect))
            old_protection = ctypes.pointer(ctypes.wintypes.DWORD())
            old_protection_state = self.get_mem_info(base_address).Protect
            # Changing the protection to PAGE_READWRITE so we can write to the memory
            result = VirtualProtectEx(
                self.process_handle, base_address, ctypes.sizeof(write_buffer),
                PAGE_READWRITE_EXECUTE, old_protection)
            if result == 0:
            ...
            if result is None or result == 0:
                raise Exception('Error: %s' % ctypes.windll.kernel32.GetLastError())
            # Restore the old protection
            result = VirtualProtectEx(
                self.process_handle, base_address, ctypes.sizeof(write_buffer),
                old_protection_state, old_protection)
            if result is None or result == 0:
                raise Exception('Error: %s' % ctypes.windll.kernel32.GetLastError())
    ...
    def list_mapped_regions(self, writeable_only: bool = True) -> List[Tuple[int, int]]:
        ...
        page_ptr = start
        while page_ptr < stop:
            page_info = self.get_mem_info(page_ptr)
